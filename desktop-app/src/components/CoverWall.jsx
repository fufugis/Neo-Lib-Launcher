...
import { CalendarDays, Clock3, Cloud, ExternalLink, Gamepad2, Grid3X3, HardDrive, Home, ImageOff, Library, List, LockKeyhole, Minus, Play, Plus, Radio, ShieldAlert, Star, Trophy, UserRound, Users, Wrench, X } from 'lucide-react';
...
export default function CoverWall({ games = [], favoriteIds = [], density = 5, onDensityChange, onOpenPreview, onLaunch, search = '', lockedCategories = [], onUnlockCategory, view = 'covers', onChangeView, onOpenHome, onOpenLibrary }) {
  ...
  return <section className="relative h-full overflow-y-auto px-5 py-4 lg:px-7" data-testid="library-cover-wall" data-wall-layout={detailed ? 'details' : 'covers'}>
    ...
    {detailed ? <WallDetails games={visible} onSelect={(id) => setPeekId((current) => current === id ? '' : id)} /> : <WallCovers games={visible} tiles={tiles} onSelect={(id) => setPeekId((current) => current === id ? '' : id)} />}
