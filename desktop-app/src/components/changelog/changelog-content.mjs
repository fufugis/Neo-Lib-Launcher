...
const ARCHIVE_CHANGELOG = [
  {
    ...
    title: 'Architecture, Reliability & FiFi',
      major: [
        {
          title: 'Release builds now fail safely when integrations are incomplete',
          body: 'GitHub refuses a missing or malformed feedback relay instead of shipping a Feedback button that cannot send. An offline relay fixture proves signing, replay and UTF-8 body limits, per-IP throttling, mention suppression, CORS, unavailable-service responses and capped Discord forwarding. If the relay is temporarily unavailable, the Feedback window keeps the typed report and offers a deliberate GitHub fallback. Final package inspection proves the configured relay reached the renderer, rejects known credential signatures and packaged .env files, and publishes installer/portable hashes without local paths or integration values. Optional Discord Rich Presence uses a numeric-only generated configuration prepared before provenance and verified inside the archive.',
        },
        {
          title: 'Smaller, more honest runtime dependency boundary',
          body: 'The obsolete electron-store production package and its shipped transitive tree are no longer declared. Library, settings and playtime already use NEO-LIB’s tested versioned document service, so the release gate now keeps native production dependencies empty until a real runtime import is deliberately introduced.',
        },
        {
          title: 'Controller Center foundation is now usable',
          body: 'Settings can open a focused Controller Center that detects gamepads only while visible, lets you choose a preferred pad and shows live button/stick input without recording input history. Pairing or removal opens the exact Windows Bluetooth page. NEO-LIB does not invent battery, wireless-strength or disconnect data the browser cannot reliably provide. Controller-wide navigation remains off until its focus and launch-safety behavior has been accepted with real hardware.',
        },
        {
          title: 'FiFi is now a selectable living companion',
          body: 'Settings can switch between Fungist and FiFi. FiFi uses the same tested alert, chat, drag, saved-dock, launch and Rest Mode host while rendering her articulated blink, gaze, breathing, fin, tendril, gesture and particle rig. Her separate 27-line local voice pool follows the existing voice mute, volume and event cooldown controls. Installed visual and listening acceptance remains pending.',
        },
        {
          title: 'Playtime ranges follow your local calendar',
          body: 'Daily snapshots, history pruning and week/month/year cutoffs now format the local calendar day directly. Local midnight in a positive UTC offset can no longer be converted into the previous date before Stats calculates its playtime delta.',
        },
        {
          title: 'CPU readings now use a current Windows-comparable window',
          body: 'Game Ready previously reused its last CPU counter snapshot, so after startup it displayed an average across the full 15-second refresh gap while Task Manager usually showed a much newer moment. NEO-LIB now takes a fresh one-second whole-machine sample for every reading. The panel labels the window and explains that rapidly changing CPU can still differ slightly between two apps viewed at different instants.',
        },
        {
          title: 'Private local crash reports you control',
          body: 'NEO-LIB now keeps a small rotating local diagnostic record for startup, renderer, native-command and process failures. It stores structured error types and safe status facts—not game names, paths, URLs, searches, request payloads, message text, keys, PINs or private-category identity. Feedback offers Copy diagnostic report and Open log folder; attaching the report remains off until the player explicitly checks it, and nothing is uploaded in the background.',
        },
        {
          title: 'Blank startup window repaired and made diagnosable',
          body: 'The mascot architecture split left one notification helper exported but not imported, so Fungist could throw immediately after mount and React would remove the entire interface. The missing binding is restored. Renderer startup now has a visible recovery screen, while the native shell records bounded local load, console and renderer-process failures. A source-wide binding gate checks every renderer module before future builds and rejects unbound runtime names.',
        },
        {
          title: 'Every packaged build now proves which source it contains',
          body: 'Settings → About now shows a build ID, architecture label, source revision and build time generated from the exact runtime source. Electron Builder verifies the same fingerprint before packaging and refuses missing or outdated interface output, preventing Visual Studio or a direct packaging command from silently combining current Windows code with an older renderer.',
        },
        {
          title: 'Release hardening now blocks architecture regressions',
          body: 'The first consolidation pass removed a shadowed HTML helper, eight unused metadata, Gemini and news helpers, and a deprecated no-output visual stub without replacing their active routes. Every renderer build now checks all runtime files for conflicting top-level declarations, broken local modules, version drift, missing release entry points/assets and returned legacy paths. A short ownership map and separate Windows acceptance, performance and rollback checklist make clear what is source-proven and what still needs a rebuilt app before release.',
        },
        {
          title: 'Operations no longer fail as endless spinners',
          body: 'Launcher scan/import, metadata review, Home and News refresh, weekly releases, game-update checks and storage scans now distinguish running, completed, partial, failed, cancelled, timed-out and unavailable work. Active manual scans can be cancelled and late superseded results are ignored. A bounded local diagnostic summary stores only operation type, status, duration, counts and error code; Feedback includes it only when the player explicitly checks the option. It contains no game names, paths, searches, keys, message text or private-category identity. Packaged Windows interaction testing remains pending.',
        },
        {
          title: 'Cleaner interface and visual architecture',
          body: 'Popup menus and modals now share one foreground gateway, decorative theme art is explicitly unable to intercept clicks, and changelog content, Settings controls, Home calculations and Fungist decisions have focused owners. A renderer-wide test parses every component and checks privacy redaction, popup ownership, decoration safety and mascot/Home behavior. Packaged Windows visual, focus and narrow-window acceptance is still pending.',
        },
        {
          title: 'Safer native command architecture completed in source',
          body: 'All 85 renderer-to-Windows commands now pass through one duplicate-safe registry and 28 explicit domain modules. All 53 commands that accept app data reject malformed or oversized requests before native work begins, and all 85 commands validate returned data against the interface shape they already use. Malformed results, service crashes and rejected background work now return safe contract-matched fallbacks. Local diagnostics identify the command and error class without recording request payloads or error text. Contracts and safe failure logging are complete; packaged Windows acceptance is still in progress.',
        },
        {
          title: 'Safer save backups and restores',
          body: 'Save inspection, backup, restore and discovery now have one independently tested native service. Its temporary-disk test exposed and fixed an old copy conflict that could reject a freshly created backup destination. Backup/restore now copy safely without overwriting existing files, restore stays confined to NEO-LIB backups, and occupied live save folders remain untouched. Packaged Windows interaction testing is still pending.',
        },
        {
          title: 'Safer storage measurement',
          body: 'Home storage measurement now has one independently tested native service. Missing targets, launcher executables, duplicate installs and misleading shared-library roots are excluded before folder measurement. Mod totals, cache reuse and forced rescans retain their existing behavior, and the scan remains user-triggered rather than a background drive crawl.',
        },
        {
          title: 'Safer Optimize junk review',
          body: 'Junk discovery and Recycle Bin actions now have one independently tested native service. Configured game and save paths stay protected; fresh files, small archives, changed files, unknown selections and stale scans are refused. Duplicate selections collapse and there is no force-delete path. Packaged Windows interaction testing remains pending.',
        },
        {
          title: 'Safer Optimize process controls',
          body: 'Windows performance inspection and cooperative process-close requests now have one independently tested native service. Critical Windows processes, NEO-LIB itself, stale lists and changed process names remain protected. An allowed close uses the exact PID without forced termination, and Windows refusal is never escalated to a force kill.',
        },
        {
          title: 'Safer game launch and external-game watching',
          body: 'Game launching and path-only external-game detection now have separate independently tested native services. Trusted one-use launch tokens, startup quarantine, local/cross-window cooldowns, exact process start, exit reporting, helper filtering and automatic low-usage resume cadence remain intact. Automated tests launch and query no real process; packaged Windows acceptance is still pending.',
        },
        {
          title: 'Store metadata now has a replaceable provider boundary',
          body: 'Steam search/details and GOG search now run through one independently tested provider service. Existing request URLs, result shapes, Steam genre evidence, cleaned descriptions, image limits and safe offline fallbacks remain unchanged. Tests use injected responses rather than the live internet; public and launcher-specific discovery now use their own tested providers too.',
        },
        {
          title: 'Public web discovery now has a tested boundary',
          body: 'DuckDuckGo and Google result parsing, game-specific query wording and fallback order now run through a separate provider service. Software/news searches remain generic while games retain their existing game-focused terms. Offline fixtures verify parsing and conservative year/genre synthesis without contacting a search engine; specialist providers now have their own tested boundary.',
        },
        {
          title: 'Specialist indie metadata now has a tested boundary',
          body: 'itch.io, DLsite, VNDB and Ryuugames lookup/parsing now run through a separate provider service. Existing identity, creator and image mapping remain intact, with offline fixtures proving safe failures without contacting those sites. Candidate orchestration is source-complete; packaged Windows acceptance remains open.',
        },
        {
          title: 'Metadata candidates now have one guarded dispatcher',
          body: 'All nine current candidate sources now pass through an explicit allow-list for result listing and expansion. Empty searches, unknown sources, malformed provider output and provider failures return safe results, while your existing Preview, selection and Apply steps remain unchanged.',
        },
        {
          title: 'Gemini requests now have a privacy-bounded adapter',
          body: 'Gemini model selection, game metadata and Fungist chat requests now have one independently tested boundary. Existing manual-send behavior, compact history, sanitized visible-library context and response limits remain intact. Tests use fake responses and send no key, prompt or library data online.',
        },
        {
          title: 'News discovery now has deterministic provider boundaries',
          body: 'News language, date, safe-link and game-match rules now have fixed-time tests. Public fallback news owns its official-site preference, DuckDuckGo/Google fallback, deduplication, two-item limit and cache without changing Steam, itch.io or GOG feed priority.',
        },
        {
          title: 'Weekly releases now use a tested discovery provider',
          body: 'The weekly feed still combines SteamSpy momentum with Steam’s New Releases shelf, verifies actual store dates and falls back from major to noteworthy or popular launches. Its seven-day window, tier thresholds, twelve-item cap, cache and offline result now have injected tests.',
        },
        {
          title: 'Owned game-news feeds now have tested providers',
          body: 'Steam news batching, caching, BBCode cleanup, images and English/date filtering now have one tested service. itch.io devlogs and GOG changelogs have another. Home keeps the same owned-feed-first order and uses public search only when those feeds are quiet.',
        },
        {
          title: 'Update-history pages now have a guarded provider',
          body: 'The manually requested update-history reader now accepts only public HTTP/HTTPS pages, bounds how much text it processes, removes duplicate versions, sorts them correctly and compares them with your installed version. Offline and invalid-page failures return safely.',
        },
        {
          title: 'Deal discovery now has one tested provider',
          body: 'Epic free games, Steam specials, Instant Gaming, GOG, Fanatical and Ubisoft deals now share one independently tested native provider. Existing source filters, limits, deal details, isolated failures and the one-hour populated cache remain unchanged; automated checks use offline fixtures only.',
        },
        {
          title: 'Update scans now coordinate through one tested service',
          body: 'Home and Preview still share an identical in-progress update scan, different library inputs wait their turn, forced refresh bypasses the short result cache and failed scans can retry cleanly. The update evidence rules are unchanged, and the coordinator tests inspect no real library.',
        },
        {
          title: 'Update pages now come through guarded discovery',
          body: 'Saved and official update pages, public Steam notes, maintained Battle.net pages and carefully matched public results now share one tested discovery service. It keeps the same source confidence, three-page limit and six-hour cache, and unrelated search results remain excluded.',
        },
        {
          title: 'Installed-version clues now have a bounded reader',
          body: 'Independent-game version clues from Blizzard files, Unity/config files, filenames and Windows resources now share one safety-tested service. It checks only nearby named files, retains the startup quarantine, and keeps executable paths as inert data rather than command text.',
        },
        {
          title: 'Update-page versions now use trusted evidence',
          body: 'Version and build numbers from public update pages now pass through one bounded evaluator. Saved and official sources outrank generic search results, numeric versions sort correctly, and search-only evidence remains available when it is genuinely the only source.',
        },
        {
          title: 'Provider and update architecture completed',
          body: 'Seventeen tested services now own metadata, news, releases, deals and update evidence instead of one growing native file. Per-game checks keep bounded requests and honest missing-evidence results; a cached page without a version can no longer appear current. Complete alpha, beta and release-candidate installed-version suffixes are retained. Offline regression and bounded live-provider checks pass; packaged Windows interaction acceptance remains pending.',
        },
        {
          title: 'Interface state now has clear owners',
          body: 'Library and category changes, Home/Library/Wall/Tools navigation, private-game hiding, metadata review queues, playtime and visual settings now live in small tested modules instead of one central screen. Home still opens first, previous Library/Tools selections are remembered, locked games are checked globally for identity leaks, and theme ambience now renders from a separate visual component without changing its appearance.',
        },
        {
    ...
    fixes: [
      'Local-data safety groundwork: library, settings and playtime saves now pass through one versioned storage system. It keeps a last-known-good backup, serializes overlapping saves, preserves private/custom fields and refuses to overwrite unreadable or newer data. Packaged migration testing is still pending.',
      'Internal maintenance: launcher detection has been separated from the main application file, with automated checks protecting the existing import behaviour. No changes to saved libraries or game launching.',
...
export const CHANGELOG = [
  ...
  V178_CHANGELOG,
  V177_CHANGELOG,
  ...ARCHIVE_CHANGELOG.filter((entry) => ![V179_CHANGELOG.version, V178_CHANGELOG.version, V177_CHANGELOG.version].includes(entry.version)),
];
