...
export default function HomeHub({ games = [], onSelect, onOpenPlaytimeImport, onOpenTidyUp, resting = false, homeLayout = {}, onUpdateHomeLayout }) {
  ...
  const [gameUpdates, setGameUpdates] = React.useState({ loading: false, items: [], checked: 0, scannedAt: 0, error: '' });
  ...
  const refreshGameUpdates = React.useCallback(async () => {
    ...
    try {
      const result = await window.api.scanGameUpdates({ games: games.map(({ id, name, appid, launcher, source, installedVersion, updateWatchUrl }) => ({ id, name, appid, launcher, source, installedVersion, updateWatchUrl })) });
      setGameUpdates({ loading: false, items: result?.items || [], checked: result?.checked || 0, scannedAt: result?.scannedAt || 0, error: result?.ok === false ? (result.error || 'Update scan unavailable.') : '' });
    } catch {
...
function getRecommendations(games, news) {
  ...
  const seen = new Set();
  return [
    updated && { game: updated.game, label: 'NEW UPDATE', reason: 'There is a new update for this game you might wanna check out.', action: 'Read update', update: true },
...
function PlayNext({ recommendations, onSelect }) {
  return <section className="mb-6 rounded-xl border border-[rgb(var(--accent)/0.35)] bg-[linear-gradient(120deg,rgb(var(--accent)/0.12),rgb(var(--panel)/0.35)_48%,rgb(var(--accent-2)/0.08))] p-4 shadow-[0_0_32px_-20px_rgb(var(--accent))]"><div className="flex items-center gap-2"><Sparkles size={15} className="text-[rgb(var(--accent-2))]" /><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[rgb(var(--accent-2))]">What should I play?</p><h2 className="text-sm font-black">A useful nudge from your own library</h2></div></div>{recommendations.length ? <div className="mt-3 grid gap-2 md:grid-cols-3">{recommendations.map(({ game, label, reason, action, update }) => <button key={game.id} onClick={() => onSelect?.(game.id)} className="group flex min-w-0 items-center gap-2.5 rounded-lg border border-[rgb(var(--border)/0.85)] bg-[rgb(var(--surface)/0.36)] p-2.5 text-left hover:border-[rgb(var(--accent)/0.55)] hover:bg-[rgb(var(--surface)/0.6)]"><Cover game={game} className="h-10 w-16" /><span className="min-w-0"><span className={`block text-[9px] font-bold uppercase tracking-wider ${update ? 'new-update-reactive text-emerald-300' : 'text-[rgb(var(--accent-2))]'}`}>{label}</span><span className="block truncate text-xs font-black group-hover:text-[rgb(var(--accent))]">{game.name}</span><span className="mt-0.5 block line-clamp-2 text-[10px] leading-snug text-muted">{reason}</span><span className="mt-1 block text-[10px] font-bold text-[rgb(var(--accent))]">{action} ›</span></span></button>)}</div> : <p className="mt-3 text-xs text-muted">Add or import a few games and NEO-LIB will begin surfacing timely reasons to play them.</p>}</section>;
}
