...
function GameUpdates({ updates, onRefresh, onSelect }) {
  ...
  return <><section className="rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--panel)/0.34)] p-4" data-testid="home-game-updates">
    ...
    {!updates.loading && updates.needsSetup?.length > 0 && <div className="mt-3 rounded-lg border border-amber-300/25 bg-amber-300/[0.045] p-3 text-[10.5px] text-muted"><b className="text-amber-200">{updates.needsSetup.length} independent game{updates.needsSetup.length === 1 ? '' : 's'} could not be compared yet.</b> NEO-LIB could not safely find an installed version or a public update page; use Customize to add the official page when one is available. <span className="mt-1 block truncate">Examples: {updates.needsSetup.slice(0, 3).map((item) => `${item.name} (${item.missing})`).join(' · ')}</span></div>}
