...
export function GameCapabilities({ game, onUpdateGame }) {
  ...
  return (
    <section className="mb-5 overflow-hidden rounded-xl border border-[rgb(var(--border)/0.7)] bg-[rgb(var(--surface)/0.16)]" data-testid="game-capabilities">
      ...
      {achievement && achievement.syncState !== 'linked' && <p className="border-t border-[rgb(var(--border)/0.45)] px-3.5 py-2 text-[9px] leading-relaxed text-muted"><Trophy size={10} className="mr-1 inline text-[rgb(var(--accent-2))]" />{achievement.source} reports achievement support{achievement.total == null ? '' : ` (${achievement.total} available)`}. {canSyncSteam ? 'Earned progress appears only after you verify and sync your Steam account.' : 'NEO-LIB cannot verify earned progress for this game yet; check its original launcher. No progress is estimated.'}</p>}
