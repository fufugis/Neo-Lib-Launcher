...
import { CircleAlert, ExternalLink, FolderSearch, RefreshCw, Users, X } from 'lucide-react';
...
export default function FriendsPanel({ manualPaths = {}, onUpdateManualPaths, resting = false, variant = 'titlebar' }) {
  ...
  const openNativeSocial = async (platform) => {
    setMessage('');
    try {
      const result = await window.api?.openLauncherSocial?.(platform.id, manualPaths[platform.id]);
      setMessage(result?.ok ? `${platform.label} opened — chats stay in the original client.` : (result?.error || `${platform.label} could not be opened.`));
...
function PlatformRow({ platform, client, onOpen, onLocate }) {
  ...
  const copy = {
    running: ['Running', 'Open Friends', 'Open the native social view.'],
    installed: ['Installed · not running', 'Start client', 'Start it, then return here to see its live status.'],
