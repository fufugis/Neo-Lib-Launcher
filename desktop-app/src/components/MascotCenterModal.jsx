...
export default function MascotCenterModal({ open, onClose, settings, setSettings }) {
  ...
  const mascotSize = Math.max(70, Math.min(145, Number(settings.fungistSize ?? 100)));
  ...
  return <Modal open={open} onClose={onClose} title="Mascot Center" wide testid="mascot-center-modal">
    <div className="p-5 space-y-4">
      ...
      <section className="rounded-xl hairline bg-[rgb(var(--surface)/0.38)] p-3" data-testid="mascot-center-size"><div className="flex items-center justify-between gap-3"><div><p className="text-[12px] font-black text-ink">Mascot size</p><p className="mt-0.5 text-[10px] text-muted">Changes the companion everywhere, including the draggable corner mascot.</p></div><span className="rounded-md border border-[rgb(var(--accent)/0.38)] px-2 py-1 text-[10px] font-black text-[rgb(var(--accent-2))]">{mascotSize}%</span></div><input aria-label="Mascot size" type="range" min="70" max="145" step="5" value={mascotSize} onChange={(event) => setKey({ fungistSize: Number(event.target.value) })} className="neo-range mt-3 w-full" /></section>
