...
- **Battle.net metadata recovery** — Re-fetch info now carries Battle.net’s local product identity all the way into NEO-LIB’s Blizzard catalogue before any cross-store match can run. Warcraft III: Reforged and World of Warcraft keep correct Blizzard title, publisher, description, website, genres, and tags even during a temporary official-page failure; a deliberate Refresh retries Blizzard’s live page for art and wording without falling into an unrelated Steam result.
- **Startup-only Fungist greeting** — “Welcome to NEO-LIB” now plays only after one completed intro per desktop window. A normal mascot notice cannot replay it as background chatter while the launcher is already open.
- **Library dropdowns now stay in front and readable** — launcher, Sort, Categories, and Refresh menus share a forced foreground layer above scrolling games, with a near-solid themed surface so names and art cannot bleed through options.
- **Battle.net Re-fetch applies exact results immediately** — Warcraft III and World of Warcraft now save a maintained Blizzard metadata result directly when Re-fetch info is pressed. Manual re-search remains a separate reviewable action.
- **All-library news intelligence** — Steam, GOG, and itch.io keep their public native feeds. EA App, Epic, Ubisoft, Battle.net, Riot, Xbox, Rockstar, standalone, and F95-style library entries now get a bounded official-site-first public-news check, followed only when necessary by an explicitly-labelled web-discovery fallback. Checks are cached and rotate through larger libraries; a search result is never treated as a verified launcher update.
- **Library popups always stay in front** — Sort, launcher filter, Categories, Add, and Refresh menus now share a dedicated foreground layer so category rows, game art, textures, and FX cannot cover them.
- **Home now tells you when private categories are locked** — a compact status notice appears while any PIN category is locked, reminding you that its game stats and news stay hidden until you unlock it in Library.
- **Preview actions now let the theme breathe** — the dark Launch/action strip uses a slightly more transparent surface and lighter blur, keeping controls readable while allowing more background FX through.
- **Private PIN setup now explains Lock private** — the first PIN setup shows the readable red-outline Lock private button at the top-right of Home, explains that it instantly re-locks every PIN category and returns to a safe Library view, and confirms that the PIN is kept for later unlocks.
- **Library toolbar stays readable over theme art** — Add, Wizard, Visuals, launcher filter, Sort, Categories, Auto-sort, and Refresh now use denser themed control surfaces, while their dropdowns are near-solid so labels and choices stay clear above textures and motion.
...
- **Outside-Library PC check** — Fungist’s PC-use warning can now take one player-requested, read-only Windows process snapshot. It names the heaviest CPU/RAM users even if the active game was never imported, and calls out clear game/client names such as Overwatch when Windows exposes them. It never reads game memory, injects, overlays, accesses accounts, or changes processes.
- **A cleaner, grouped Home** — Top 5 Played remains fixed at the top, with a smaller **This Week’s News** rail directly beneath it. The rest of Home is now arranged into **Play & history**, **News & updates**, and **Library & PC care**, with deliberate visual separation while scrolling. Drag cards only within their own section; drag a section itself to rearrange whole groups without mixing unrelated content.
- **Home makes better use of the screen** — Recent Sessions pairs with My Best Games, Your Week pairs with Gaming Chronicle, and Library Health pairs with Storage Control on a wide launcher. Every card remains individually reorderable/hideable; dragging shows a live horizontal or vertical drop line before release. Compact windows fall back to one clean column.
...
- **Released This Week has a real quiet-week fallback** — Major launches remain first. If there are none, NEO-LIB tries noteworthy releases with early momentum, then clearly-labelled popular new releases from Steam’s current New Releases shelf instead of leaving the panel empty.
- **External-game low-usage handoff** — a Fungist high/elevated-use alert can now run a player-requested local scan for a known NEO-LIB game started from Steam, Battle.net, Epic, or another client. A match names the game and offers **Enable low usage until it closes**; after you choose it, NEO-LIB rests until that exact process exits, then wakes automatically. It reads only ordinary executable paths—never accounts, game memory, overlays, injection, or network data.
- **Library-only font choice** — Visuals now provides five system font stacks, plus **Fat** and **Cursive** toggles, strictly for Library game/category text. Home, Preview, Tools, and Settings are unchanged.
- **Proper Special-theme art** — transparent Anime blossom-vine, Industrial machinery/hazard, and Magical rune-light illustrations now replace procedural decoration fragments. They render on a deliberate UI-safe composition layer, remain visibly present at normal FX levels, stay pointer-safe and slider-controlled, and disappear in Rest Mode.
- **Full Special-theme button frames** — the main Home/Library/Wall/Tools button bank and Preview action strip now use purpose-made transparent artwork: a complete blossom-vine surround for Anime, gears/metal/hazard-light frame for Industrial, and pink/cyan rune-and-crystal frame for Magical. The art is visibly present but remains behind labels and hit targets.
...
- **No more FX fog over controls** — the permanent edge glow now sits behind the interface rather than blending over buttons and labels. It never stole clicks, but it could make controls look veiled; that visual layering bug is removed.
- **Lighter default Library type** — normal Library typography now uses a true regular weight rather than a semi-bold baseline. Use **Fat** only when you want stronger, heavier game and category names.
...
- **External-launch Rest Mode detection** — a known library game launched from Steam, Battle.net, Epic, or another client can be identified through an exact ordinary Windows process-path match. An idle launcher never counts; NEO-LIB does no injection, overlay, memory inspection, or credential access. The current high-use flow offers a clear one-game low-usage choice before NEO-LIB pauses.
