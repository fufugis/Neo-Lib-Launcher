...
assert.equal(normalizeVersion('v1.7.8'), '1.7.8');
assert.equal(normalizeVersion('v.1.7.5'), '1.7.5', 'current installs must tolerate the historical dotted tag');
assert.equal(isNewerVersion('v1.7.8', '1.7.3'), true);
assert.equal(isNewerVersion('v.1.7.5', '1.7.3'), true);
assert.equal(isNewerVersion('v1.7.8', '1.7.8'), false);
assert.equal(isNewerVersion('v1.7.7', '1.7.8'), false);

const releaseTag = releaseTagForVersion(packageJson.version);
assert.equal(releaseTag, 'v1.7.8', 'this release must use the clean legacy-compatible tag');
assert.equal(isLegacyCompatibleReleaseTag(releaseTag, packageJson.version), true);

// v1.7.3 used the simple parser below. A clean v1.7.8 tag is deliberately
