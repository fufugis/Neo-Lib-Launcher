...
const releaseNotes = fs.readFileSync(path.join(repoRoot, 'RELEASE_NOTES_v1.7.7.md'), 'utf8');
const windowsAcceptance = fs.readFileSync(path.join(repoRoot, 'WINDOWS_ACCEPTANCE_V1.7.7.md'), 'utf8');
...
assert(windowsWorkflow.includes('body_path: RELEASE_NOTES_v1.7.7.md'), 'GitHub release must use the reviewed v1.7.7 release notes');
for (const heading of ['A rebuilt foundation', 'Meet FiFi', 'Private games stay private everywhere', 'Trustworthy builds and updates', 'Release status']) {
  assert(releaseNotes.includes(heading), `v1.7.7 release notes are missing ${heading}`);
}
...
const currentChangelogSource = read('src/components/changelog/v177-changelog.mjs');
...
assert(changelogSource.includes("import { V177_CHANGELOG } from './v177-changelog.mjs'"), 'displayed changelog must use the curated current release entry');
assert(changelogSource.includes("entry.version !== V177_CHANGELOG.version"), 'archived draft entry must not duplicate the displayed current release');
