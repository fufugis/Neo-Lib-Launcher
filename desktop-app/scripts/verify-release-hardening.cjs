...
const releaseNotes = fs.readFileSync(path.join(repoRoot, 'RELEASE_NOTES_v1.8.0.md'), 'utf8');
const windowsAcceptance = fs.readFileSync(path.join(repoRoot, 'WINDOWS_ACCEPTANCE_V1.8.0.md'), 'utf8');
...
assert(windowsWorkflow.includes('body_path: RELEASE_NOTES_v1.8.0.md'), 'GitHub release must use the reviewed v1.8.0 release notes');
for (const heading of ['One Library Wizard', 'Full-workspace Wall', 'Two Wall views', 'Readable Wall polish', 'Release status']) {
  assert(releaseNotes.includes(heading), `v1.8.0 release notes are missing ${heading}`);
}
for (const requiredAcceptanceStep of [
  '104 native commands have one registration and request/response contracts.',
...
const currentChangelogSource = read('src/components/changelog/v180-changelog.mjs');
...
assert(changelogSource.includes("import { V180_CHANGELOG } from './v180-changelog.mjs'"), 'displayed changelog must use the curated current release entry');
assert(changelogSource.includes('V180_CHANGELOG,'), 'displayed changelog must include the current release');
