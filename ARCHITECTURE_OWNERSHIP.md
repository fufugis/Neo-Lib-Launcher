...
| Library experience contracts | `desktop-app/src/lib/game-journey-model.mjs`, `game-launch-routes-model.mjs`, `game-signals-model.mjs`, `artwork-revision-model.mjs`, `desktop-app/src/components/library/wall-columns-model.mjs` | Owns Journey Status, bounded Launch Routes, evidence-led Game Signals, artwork-slot history/locks and Detailed Wall column layout before their staged interfaces are connected. These models perform no launch, filesystem or network work. |
