...
That package predates the current v1.7.8 source and must not be presented as the
architecture-finished candidate. The next accepted artifacts require a fresh build,
new fingerprint/checksum and the exact clean GitHub release tag `v1.7.8`. This clean
tag is part of backward compatibility: installed v1.7.3 recognizes `v1.7.8`, while
its historical parser does not recognize the dotted `v.1.7.5` form.

## Automated source gate

The current v1.7.8 build identity is generated from source at build time and carries
...
installer, portable ZIP and archive SHA-256 evidence in `dist/release-candidate-v1.7.8.json`.
...
Record the human checks in `WINDOWS_ACCEPTANCE_V1.7.8.md`.
